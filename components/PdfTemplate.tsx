...
export const PdfTemplate: React.FC<PdfTemplateProps> = ({ client, cart, subtotal, discount, total, shippingType, cotNumber }) => {
  ...
  // SISTEMA DE PAGINACIÓN INTELIGENTE Y SEGURO
  const ITEMS_FIRST_ONLY_PAGE = 7; // Si solo hay 1 página (tiene header grande, totales y footer)
  const ITEMS_FIRST_PAGE = 12; // Si hay múltiples páginas, la primera tiene header grande pero NO footer
  const ITEMS_MIDDLE_PAGE = 16; // Páginas intermedias (mini header, sin footer)
  const ITEMS_LAST_PAGE = 6; // Última página (mini header, totales, footer)

  const cartChunks = [];
  let remaining = [...cart];

  // Caso 1: Todo cabe en una sola página (Primera y Última a la vez)
  if (remaining.length <= ITEMS_FIRST_ONLY_PAGE) {
    cartChunks.push(remaining);
    remaining = [];
  } 
  // Caso 2: Múltiples páginas
  else {
    // Llenar la primera página. Aseguramos dejar al menos 1 item para las siguientes páginas.
    let takeFirst = Math.min(ITEMS_FIRST_PAGE, remaining.length - 1);
    cartChunks.push(remaining.slice(0, takeFirst));
    remaining = remaining.slice(takeFirst);

    // Llenar páginas intermedias y la última
    while (remaining.length > 0) {
      if (remaining.length <= ITEMS_LAST_PAGE) {
        // Cabe perfectamente en la última página
        cartChunks.push(remaining);
        break;
      } else {
        // Llenar una página intermedia. Aseguramos dejar al menos 1 item para la última página.
        let takeMiddle = Math.min(ITEMS_MIDDLE_PAGE, remaining.length - 1);
        cartChunks.push(remaining.slice(0, takeMiddle));
        remaining = remaining.slice(takeMiddle);
      }
    }
  }

  // Paginación del Anexo Visual
  const anexoChunks = [];
  for (let i = 0; i < cart.length; i += 6) {
    anexoChunks.push(cart.slice(i, i + 6));
  }

  return (
    <div id="pdf-template" className="flex flex-col gap-8">
      
      {/* RENDERIZADO DE PÁGINAS DE LA COTIZACIÓN */}
      {cartChunks.map((chunk, pageIndex) => {
        const isFirstPage = pageIndex === 0;
        const isLastPage = pageIndex === cartChunks.length - 1;

        return (
          <div key={`page-${pageIndex}`} className="pdf-section w-[794px] h-[1123px] bg-white text-slate-800 font-sans mx-auto shadow-2xl flex flex-col relative overflow-hidden shrink-0">
            
            {/* HEADER CORPORATIVO (Solo página 1) */}
            {isFirstPage && (
              <div className="bg-slate-900 text-white px-10 py-8 relative shrink-0">
            ...
            {/* MINI HEADER (Páginas siguientes) */}
            {!isFirstPage && (
              <div className="bg-slate-900 text-white px-10 py-4 flex justify-between items-center shrink-0">
                 <span className="font-bold tracking-wider">CYLO - COTIZACIÓN {cotNumber}</span>
                 <span className="text-xs text-slate-300">Página {pageIndex + 1} de {cartChunks.length}</span>
              </div>
            )}

            {/* METADATOS DEL CLIENTE (Solo página 1) */}
            {isFirstPage && (
              <div className="px-10 py-6 bg-slate-50 border-b border-slate-200 flex justify-between shrink-0">
            ...
            {/* FOOTER CONDICIONES (Solo última página) */}
            {isLastPage && (
      ...
      {/* RENDERIZADO DE PÁGINAS DEL ANEXO VISUAL */}
      {anexoChunks.length > 0 && anexoChunks.map((chunk, pageIndex) => (
        <div key={`anexo-${pageIndex}`} className="pdf-section w-[794px] h-[1123px] bg-white text-slate-800 font-sans mx-auto shadow-2xl flex flex-col relative overflow-hidden shrink-0">
          ...
          <div className="bg-slate-900 text-white px-10 py-8 relative shrink-0">
            ...
            <div className="flex justify-between items-center relative z-10">
              <div>
                ...
                <h4 className="text-[10px] font-bold tracking-[0.3em] text-indigo-300">
                  CATÁLOGO DE PRODUCTOS COTIZADOS {anexoChunks.length > 1 ? `(Pt. ${pageIndex + 1})` : ''}
                </h4>
              </div>
              <div className="text-right">
                <p className="font-mono font-bold text-indigo-400 mt-1">{cotNumber}</p>
          ...
          <div className="mt-auto bg-slate-900 h-6 w-full shrink-0"></div>
        </div>
      ))}
    </div>
  );
